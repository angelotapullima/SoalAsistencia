...
class AsistenciaApi {
  ...
  Future<bool> guardarAsistencia() async {
    try {
      final url = Uri.parse(
          'https://intranet.proonix.com.pe/api/bufeo/guardar_asistencia_diaria');

      final String fecha = obtenerFecha();

      var personas = '';

      final listAsistencias =
          await _asistenciaDatabase.obtenerAsistenciasFechaPersonas(fecha);

      for (var i = 0; i < listAsistencias.length; i++) {
        personas +=
            '${listAsistencias[i].tipoDoc};${listAsistencias[i].nroDoc};${listAsistencias[i].hora}|';
      }
  ...
  Future<bool> obtenerPersonas() async {
    try {
      ...
      final url = Uri.parse(
          'https://intranet.proonix.com.pe/api/bufeo/listar_miembros_asistencia');
      ...
      if (decodedData['result']['code'] == 1) {
        for (var i = 0; i < decodedData['result']['personal'].length; i++) {
          ...
          persona.personName =
              decodedData['result']['personal'][i]['person_name'];
          persona.personSurname =
              decodedData['result']['personal'][i]['person_surname'];
          persona.personSurname2 =
              decodedData['result']['personal'][i]['person_surname2'];
          persona.personTipoDoc =
              decodedData['result']['personal'][i]['person_type_document'];
          persona.personNroDoc =
              decodedData['result']['personal'][i]['person_dni'];
